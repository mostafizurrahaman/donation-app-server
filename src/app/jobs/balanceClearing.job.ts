...
export const balanceClear = async () => {
  console.log(`🔄 Starting Balance Clearing Job: ${new Date().toISOString()}`);
  cronJobTracker.startExecution(JOB_NAME);
      ...
      try {
        ...
        if (unclearedTransactions.length > 0) {
          ...
          for (const tx of unclearedTransactions) {
            ...
            if (tx.donationType === 'one-time') {
              clearedOneTime = Number((clearedOneTime + tx.amount).toFixed(2));
            } else if (tx.donationType === 'recurring') {
              clearedRecurring = Number(
                (clearedRecurring + tx.amount).toFixed(2)
              );
            } else if (tx.donationType === 'round-up') {
              clearedRoundUp = Number((clearedRoundUp + tx.amount).toFixed(2));
            }
          ...
          if (clearedAmount > 0) {
            ...
            balance.availableByType_recurring = Number(
              (balance.availableByType_recurring + clearedRecurring).toFixed(2)
            );
              ...
              [
                {
                  ...
                  idempotencyKey: `clear_${balance.organization}_${Date.now()}`,
                },
              ],
              { session }
            );
